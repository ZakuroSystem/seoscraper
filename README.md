...
* Flask を用いた HTML Web UI（ポート5007）に対応
...
* 編集距離に基づき類似キーワードを統合し長い語形を代表として集計（閾値指定可）
...
* 生成されたレポートとブログ記事は Markdown (`.md`) として保存され、Web UI からプレビューおよびダウンロードが可能
* Web UI は検索→レポート生成→ブログ生成の順にボタンが現れる段階的な操作で、各結果は Markdown プレビューとダウンロードリンクを提供。AI 呼び出しのログも画面に表示
...
* `--merge-percent` : 類似キーワードを統合する最大編集距離パーセンテージ（デフォルト 0）
...
検索結果と共通フレーズを集約し、ローカルの Ollama `gpt-oss:20b` モデルが利用可能な場合は SEO ブログ記事を書くための指示書を生成し、続けて本文も自動執筆します。接続先は環境変数 `OLLAMA_API_BASE`（デフォルト `http://localhost:11434`）で変更でき、モデル有無のチェック結果はキャッシュされます。生成された記事は Markdown 形式で `static/blogs/` 配下に保存され、Web UI からダウンロードできます。モデルが見つからない場合は自動的にスキップされます。
...
ブラウザで [http://localhost:5007](http://localhost:5007) にアクセスしてください。フォームから遅延やワーカー数、解析モード、除外パターンなど主要な機能を設定できます。検索実行後の結果はフォームより上部に表示され、本文は50文字まで表示されます。クリックすると全文が展開されます。
個別ページの詳細は「ページ別の詳細を表示」をクリックすると展開されます。
まず「実行」ボタンでスクレイピングのみを行い、結果表示後に現れる「レポート作成」ボタンで分析レポートを生成します。レポート生成後は Markdown プレビューとダウンロードリンクが表示され、続けて「ブログ作成」ボタンでブログ記事を生成できます。ブログ生成後も同様にプレビューとダウンロードリンクが表示され、生成過程のログは結果の下に表示されます。
