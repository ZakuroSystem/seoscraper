...
Janome を用いて本文を形態素解析し、名詞のみを対象に語数と出現頻度上位のキーワードを算出します。助詞・助動詞などのストップワードは自動的に除外されます。CLI / Web UI で表示され、CSV や JSON の結果にも `word_count` と `top_keywords` フィールドとして保存されます。
### Web UI
