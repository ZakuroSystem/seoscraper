...
* ローカルの Ollama `gpt-oss:20b` が利用可能なら検索結果からブログ指示書を自動生成（未インストール時はスキップ）
...
検索結果と共通フレーズを集約し、ローカルの Ollama `gpt-oss:20b` モデルが利用可能な場合は SEO ブログ記事を書くための指示書を自動生成します。モデルが見つからない場合は自動的にスキップされます。
