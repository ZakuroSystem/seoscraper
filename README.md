...
* 生成されたブログ記事は Markdown (`.md`) として `static/blogs/` 配下に保存され、Web UI からダウンロード可能
* Web UI は検索→レポート生成→ブログ生成の順にボタンが現れる段階的な操作で、各結果は Markdown としてダウンロード可能。AI 呼び出しのログも画面に表示
...
まず「実行」ボタンでスクレイピングのみを行い、結果表示後に現れる「レポート作成」ボタンで分析レポートを生成します。レポート生成後は Markdown ダウンロードリンクが表示され、続けて「ブログ作成」ボタンでブログ記事を生成できます。生成過程のログは結果の下に表示されます。
