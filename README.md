...
* Web UI 上でレポート生成とブログ生成をボタンで選択でき、AI 呼び出しのログも画面に表示
...
ブラウザで [http://localhost:5007](http://localhost:5007) にアクセスしてください。フォームから遅延やワーカー数、解析モード、除外パターンなどすべての主要機能を直感的に設定できます。検索実行後の結果はフォームより上部に表示され、本文は50文字まで表示されます。クリックすると全文が展開されます。
個別ページの詳細は「ページ別の詳細を表示」をクリックすると展開されます。
ブログを生成したい場合は「ブログ作成」ボタンを、解析レポートのみの場合は「レポート作成」ボタンを押してください。生成過程のログは結果の下に表示されます。
