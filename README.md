...
* ローカルの Ollama `gpt-oss:20b` モデルを自動検出し、利用可能なら検索結果からブログ指示書と本文を生成（見つからない場合はスキップ）
...
検索結果と共通フレーズを集約し、ローカルの Ollama `gpt-oss:20b` モデルが利用可能な場合は SEO ブログ記事を書くための指示書を生成し、続けて本文も自動執筆します。生成された記事は Markdown 形式で `static/blogs/` 配下に保存され、Web UI からダウンロードできます。モデルが見つからない場合は自動的にスキップされます。
